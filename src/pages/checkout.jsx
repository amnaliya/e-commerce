...
return (
    ...
    <div className="bg-[#f7f3eb] min-h-screen px-4 py-8 sm:px-6 sm:py-10 lg:px-8">
        <h1 className="text-center font-bold text-3xl text-[#5c4033] mb-6 py-1 sm:mb-8 sm:text-4xl ">CHECK OUT </h1>
        ...
        <div className="mx-auto max-w-6xl grid gap-6 grid-cols-1 gap-6 lg:grid-cols-[1fr_380px] lg:gap-8">
            ...
            <h2 className="font-bold  text-[#5c4033] text-xl sm:text-2xl  ">Shipping Details </h2>
            <div>
                <form className="space-y-3 mt-5">
                    ...
                    />
                        <input className="w-full rounded-md border border-gray-300 p-3 outline-none transition focus:border-[#5c4033] sm:text-base"
                        type="email" placeholder="enter your email" value={email} 
                        onChange={(e)=>setemail(e.target.value)} />
                
                        <textarea className="w-full rounded-md border border-gray-300 p-3 outline-none transition focus:border-[#5c4033] sm:text-base" rows={3} cols={5}
                 ...
                 <div className="h-fit rounded-xl p-4 sm:p-6 lg:sticky lg:top-24 lg:p-7 shadow-md bg-white">
                        <h2 className="mb-5  text-xl sm:text-2xl font-bold text-[#5c4033]">ORDER SUMMARY</h2>
                        <div className="mb-6 space-y-4">
                            {cart.map((product)=>(
                                <div className="flex items-start  gap-3 justify-between border-b pb-3"
                                key={product.id}>
                                
                                        <h3 className="text-[#5c4033] font-semibold break-words ">{product.name}</h3>
                                        <p className="text-sm text-gray-500 ">Quantity:{product.quantity}</p>
                                        <p className=" text-[#5c4033] font-medium shrink-0"> ₹{product.price * product.quantity}</p>
                                        </div>
                            ...
                            <p className="font-bold text-xl sm:text-2xl text-[#5c4033]">TOTAL:₹{total}</p>
                            </div>
                            <div className="flex justify-center  mt-7">
                        <button className="mt-6 w-full rounded-md bg-[#5c4033] text-sm py-3 font-semibold text-white sm:mt-7 sm:text-base"
                        onClick={handleorder}>
